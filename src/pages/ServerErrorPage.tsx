...
export const ServerErrorPage: React.FC<ServerErrorPageProps> = ({
  ...
  const handleCopy = () => {
    const data = {
      type: 'HTTP_500_SERVER_ERROR',
      timestamp: new Date().toISOString(),
      url: window.location.href,
      message: error?.message || 'Upstream server gateway returned 500',
    ...
    <div className="relative min-h-screen w-full bg-[#07090C] text-[#E8DCC4] flex flex-col justify-between font-sans selection:bg-rose-500/30 selection:text-white overflow-hidden">
      ...
      <header className="relative z-20 px-6 sm:px-12 lg:px-16 py-6 flex items-center justify-between">
        ...
        >
          ...
          <span className="font-display font-semibold tracking-wider text-white text-sm sm:text-base flex items-center gap-1.5">
            OSTRA <span className="text-[10px] font-mono uppercase px-1.5 py-0.5 rounded bg-rose-500/20 text-rose-300 border border-rose-500/30">500 Gateway Fault</span>
          </span>
        </div>
      ...
      <main className="relative z-20 flex-1 flex flex-col justify-center items-end px-6 sm:px-12 lg:px-24 py-8">
        <div className="w-full max-w-lg lg:max-w-xl flex flex-col items-center lg:items-start text-center lg:text-left">
          ...
          {/* Heading */}
          <h1 className="text-7xl sm:text-8xl lg:text-9xl font-display font-extrabold tracking-tight bg-gradient-to-b from-rose-200 via-rose-300 to-rose-600 bg-clip-text text-transparent drop-shadow-[0_6px_30px_rgba(244,63,94,0.4)] select-none">
            500
          </h1>
          ...
          <p className="text-stone-300 text-sm sm:text-base leading-relaxed mb-8 max-w-md font-sans drop-shadow">
            The upstream gateway cluster encountered an unexpected disruption. Failover circuits were triggered to maintain database integrity.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3.5 w-full sm:w-auto">
            <button
              onClick={onRetry}
              className="w-full sm:w-auto px-8 py-3.5 rounded-full border border-rose-500/60 hover:border-rose-400 bg-black/40 hover:bg-rose-500/15 backdrop-blur-md text-rose-300 hover:text-white font-medium text-sm sm:text-base flex items-center justify-center gap-2.5 transition-all duration-300 shadow-[0_0_25px_rgba(244,63,94,0.25)] hover:scale-105 active:scale-95 group"
            >
          ...
          <div className="mt-8 pt-4 border-t border-white/10 w-full">
            ...
            >
              <span className="flex items-center gap-2">
                ...
                <span>Inspect Server Diagnostics</span>
              </span>
          ...
          {showLogs && (
            <div className="w-full mt-4 rounded-2xl bg-black/80 backdrop-blur-xl border border-white/10 p-4 text-left shadow-2xl animate-fadeIn">
              <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/10">
                <span className="text-[11px] font-mono text-rose-400">Gateway Telemetry</span>
                <button
                  onClick={handleCopy}
              ...
              <pre className="text-stone-300 text-[11px] font-mono whitespace-pre-wrap max-h-40 overflow-y-auto">
                {error?.message || 'Error: Upstream gateway connection failure (500 Internal Fault)'}
                {'\n\n'}
                {error?.stack || 'POST /api/v1/telemetry/dispatch HTTP/2.0\nStatus: 500\nZone: us-east-core'}
              </pre>
            </div>
          )}
