...
import { TraceRepository, type LocalTraceRecord } from '../src/db/repository';
...
async function runStage1Verification() {
  ...
  const sampleTrace: LocalTraceRecord = {
    id: 'trc_001',
    requestId: 'req_001',
  ...
  const errorTrace: LocalTraceRecord = {
    ...sampleTrace,
    id: 'trc_002',
    requestId: 'req_002',
    statusCode: 429,
  ...
  const ring = new RingBuffer<{ name: string }>(200);
