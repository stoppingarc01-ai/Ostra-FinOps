...
import { TraceRepository, type LocalTraceRecord } from '../src/db/repository.js';
...
async function runChaosAndResilienceChecks() {
  ...
  try {
    ...
    let allFinishedResolve: () => void;
    const allFinishedPromise = new Promise<void>((resolve) => {
      allFinishedResolve = resolve;
    });

    const clientBuffers = new Array(clientCount).fill('');
    const clientsConnectedPromise = Promise.all(
      Array.from({ length: clientCount }, (_, i) => {
        return new Promise<void>((resolveConnect) => {
          const sseReq = http.request(
            `http://127.0.0.1:${port}/api/stream?token=${token1}`,
    ...
    for (let i = 0; i < clientCount; i++) {
      assert.strictEqual(
        ...
        burstCount,
        `Client ${i} missed events: got ${clientReceivedCounts[i]}/${burstCount}`
      );
    }
    ...
    const page2Res = await requestHttp(
      `http://127.0.0.1:${port}/api/traces?token=${token1}&limit=15&cursor=${p1Data.nextCursor}`
    );
    ...
    const p1Ids = new Set(p1Data.traces.map((t: any) => t.id));
