...
import { handleLocalIngress } from '../proxy/local-ingress.js';
...
export function createDaemonServer(options) {
    ...
    const tasksRepository = options.tasksRepository ||
    ...
    ].filter((d) => typeof d === 'string' && fs.existsSync(d));
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (cleanUrl === '/v1/chat/completions' ||
        ...
        if (resolvedStaticDir && serveStaticFile(req, res, resolvedStaticDir)) {
            return;
        }
    ...
    return {
        ...
        tasksRepository,
        config: options.config,
        listen(port = options.config.uiPort, host = options.config.bindHost, proxyPort = options.config.proxyPort) {
