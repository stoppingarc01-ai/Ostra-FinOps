...
import { getAccountPayload, updateAccountPreferences, switchActiveVirtualKey, unlinkMachineCredentials, pairMachineCredentials, } from '../credentials.js';
export function sendJson(res, statusCode, data, origin, extraHeaders) {
...
export function handleApiRoute(req, res, ctx) {
    ...
    if (pathname === '/api/tasks' && req.method === 'GET') {
        ...
        const priorityParam = urlObj.searchParams.get('priority');
    ...
    if (pathname === '/api/tasks' && req.method === 'POST') {
        ...
        parseJsonBody(req)
            ...
            const status = ['todo', 'in_progress', 'completed'].includes(body.status) ? body.status : 'todo';
    ...
    if (pathname.startsWith('/api/tasks/') && req.method === 'PATCH') {
        ...
        parseJsonBody(req)
            ...
            if (typeof body.targetDate === 'string')
                updates.targetDate = body.targetDate.trim();
            if (typeof body.targetTime !== 'undefined')
                updates.targetTime = body.targetTime ? String(body.targetTime).trim() : null;
            if (typeof body.tokenBudgetUsd === 'number')
                updates.tokenBudgetUsd = body.tokenBudgetUsd;
