 ...
 */
export async function handleLocalIngress(req, res, ctx) {
    ...
    if (isOpenAI) {
        ...
        if (req.headers['openai-organization']) {
            upstreamHeaders['OpenAI-Organization'] = req.headers['openai-organization'];
        }
        if (req.headers['openai-project']) {
            upstreamHeaders['OpenAI-Project'] = req.headers['openai-project'];
        }
    }
    else {
        // Anthropic
        ...
        if (clientKey) {
            upstreamHeaders['x-api-key'] = clientKey;
        }
                ...
                while (true) {
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            if (dataObj.model)
                                routedModel = dataObj.model;
                            if (isOpenAI) {
                                // OpenAI streaming usage or delta
                                if (dataObj.usage) {
                                    if (dataObj.usage.prompt_tokens)
                                        inputTokens = dataObj.usage.prompt_tokens;
                                    if (dataObj.usage.completion_tokens)
                                ...
                                if (dataObj.type === 'message_start' && dataObj.message) {
                                    if (dataObj.message.model)
                                        routedModel = dataObj.message.model;
 ...
 */
function recordTrace(ctx, trace) {
    try {
