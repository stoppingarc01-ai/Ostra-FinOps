...
export class RollingVelocityEngine {
     ...
     */
    record(inputTokens, outputTokens, timestamp = Date.now()) {
    ...
    getMetrics(now = Date.now()) {
        ...
        const currentBucket = this.buckets.find((b) => b.bucketStartMs === Math.floor(now / BUCKET_WIDTH_MS) * BUCKET_WIDTH_MS);
